...
public final class HudScreenRenderer {
    ...
    private static void blitFull(GuiGraphics guiGraphics, Identifier tex, int x, int y, int w, int h) {
        guiGraphics.blit(RenderPipelines.GUI_TEXTURED, tex, x, y, 0.0F, 0.0F, w, h, w, h, w, h, -1);
    }

    private static void blitTexture(GuiGraphics guiGraphics, Identifier tex, int x, int y, int w, int h,
            int texW, int texH) {
        guiGraphics.blit(RenderPipelines.GUI_TEXTURED, tex, x, y, 0.0F, 0.0F, w, h, texW, texH, texW, texH, -1);
    }

    private static void blitRegion(GuiGraphics guiGraphics, Identifier tex,
    ...
    private static void drawProgressBar(GuiGraphics guiGraphics, int x, int y, int w, int h, RenderState state) {
        ...
        switch (state.playbackState) {
            case LOADING -> {
                ...
                if (blockLeft > x) {
                    drawColoredRect(guiGraphics, x, barY, blockLeft - x, barHeight, 0x40000000);
                }
                if (blockRight > blockLeft) {
                    drawColoredRect(guiGraphics, blockLeft, barY, blockRight - blockLeft, barHeight, 0xCCFFFFFF);
                }
                if (blockRight < x + w) {
                    drawColoredRect(guiGraphics, blockRight, barY, x + w - blockRight, barHeight, 0x40000000);
                }
            }

...
public final class HudScreenRenderer {
    ...
    public static void render(HudScreenPeripheral peripheral, GuiGraphics guiGraphics) {
        ...
        ResourceLocation foreground = resolveForegroundTexture(state);
    ...
    private static void blitFull(GuiGraphics guiGraphics, ResourceLocation tex, int x, int y, int w, int h) {
        guiGraphics.blit(RenderPipelines.GUI_TEXTURED, tex, x, y, 0.0F, 0.0F, w, h, w, h, w, h, -1);
    }

    private static void blitTexture(GuiGraphics guiGraphics, ResourceLocation tex, int x, int y, int w, int h,
            int texW, int texH) {
        guiGraphics.blit(RenderPipelines.GUI_TEXTURED, tex, x, y, 0.0F, 0.0F, w, h, texW, texH, texW, texH, -1);
    }

    private static void blitRegion(GuiGraphics guiGraphics, ResourceLocation tex,
    ...
    private static RenderState loadState(HudScreenPeripheral peripheral) {
        ...
        ResourceLocation backgroundTextureId = peripheral.getBackgroundTextureId();
        ...
        int textureWidth = 0;
        int textureHeight = 0;
        @Nullable ResourceLocation textureId = null;
    ...
    private static @Nullable ResourceLocation resolveForegroundTexture(RenderState state) {
    ...
    private static void drawBackgroundStrips(GuiGraphics guiGraphics, int x, int y, int w, int h, RenderState state, Bounds fg) {
        int bw = state.textureWidth > 0 ? state.textureWidth : w;
        int bh = state.textureHeight > 0 ? state.textureHeight : h;
        ResourceLocation tex = state.backgroundTextureId;
    ...
    // ---- foreground bounds (aspect ratio) ----
    ...
    private static void drawProgressBar(GuiGraphics guiGraphics, int x, int y, int w, int h, RenderState state) {
    ...
    private static void drawProgressFill(GuiGraphics guiGraphics, int x, int barY, int w, int barH, float progress, int color) {
        if (progress > 0.0F) {
    ...
    private static void drawColoredRect(GuiGraphics guiGraphics, int x, int y, int w, int h, int color) {
        guiGraphics.fill(x, y, x + w, y + h, color);
    }

    // ---- status text ----

    private static void drawStatusText(GuiGraphics guiGraphics, int x, int y, int w, int h, RenderState state) {
        String text;
        int color;
    ...
    private record RenderState(
            ...
            @Nullable ResourceLocation backgroundTextureId,

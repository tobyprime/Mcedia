...
public class PlayerRenderer {
    ...
    // --- 主渲染方法 ---
    public void render(ArmorStand entity, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, PlayerAgent agent) {
        MediaPlayer player = agent.getPlayer();
        PlayerConfigManager config = agent.getConfigManager();
        Media media = player.getMedia();

        boolean showBasePlate = !entity.isNoBasePlate();
        float scale = entity.getScale();
        Rotations leftArmPose = entity.getLeftArmPose();
        ...
        if (media != null) {
            ...
            if (showBasePlate && media.isPaused() && agent.isPausedByBasePlate()) {
                player.play();
                agent.setPausedByBasePlate(false);
            } else if (!showBasePlate && !media.isPaused()) {
                player.pause();
                agent.setPausedByBasePlate(true);
            }
        }
        var size = scale * config.scale;

        var leftArmPoseX = leftArmPose.getX();
        var leftArmPoseY = leftArmPose.getY();

        var volumeFactor = 1 - (leftArmPoseX < 0 ? -leftArmPoseX / 360f : (360.0f - leftArmPoseX) / 360f);
        var speedFactor = leftArmPoseY < 0 ? -leftArmPoseY / 360f : (360.0f - leftArmPoseY) / 360f;
        float speed = speedFactor < 0.1f ? 1f : (speedFactor > 0.5f ? 1f - (1f - speedFactor) * 2f : (speedFactor - 0.1f) / 0.4f * 8f);
        player.setSpeed(speed);

        float yRotRadians = (float) -Math.toRadians(entityYaw);
        ...
        agent.getPrimaryAudioSource().setPos(((float) x + primaryAudioOffsetRotated.x), ((float) y + primaryAudioOffsetRotated.y), ((float) z + primaryAudioOffsetRotated.z));
        ...
        if (config.isSecondarySourceActive) {
            ...
            agent.getSecondaryAudioSource().setPos(((float) x + secondaryAudioOffsetRotated.x), ((float) y + secondaryAudioOffsetRotated.y), ((float) z + secondaryAudioOffsetRotated.z));
        }
        ...
        if (config.customLightLevel != -1) {
            ...
            finalLightValue = packedLight;
        }

        poseStack.pushPose();
        try {
            poseStack.mulPose(new Quaternionf().rotationYXZ((float) Math.toRadians(-entityYaw), 0, 0));

            poseStack.mulPose(new Quaternionf().rotationYXZ(
                    (float) Math.toRadians(-headPose.getX()),
                    (float) Math.toRadians(-headPose.getY()),
                    (float) Math.toRadians(-headPose.getZ())
            ));

            poseStack.translate(config.offsetX, config.offsetY + 1.02 * scale, config.offsetZ + 0.6 * scale);

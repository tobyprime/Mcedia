...
public class PlayerAgent {
    ...
    @Nullable private VideoTexture texture = null;
    private float audioRangeMin = 2;
    private float audioRangeMax = 500;
    String inputContent = null;
    public float speed = 1;
    private String desiredQuality = "自动";
    public PlayerAgent(ArmorStand entity) { LOGGER.info("在 {} 注册了一个 Mcedia Player 实例", entity.position()); this.entity = entity; player = new MediaPlayer(); player.setDecoderConfiguration(new DecoderConfiguration(new DecoderConfiguration.Builder())); player.bindAudioSource(audioSource); }
    public void initializeGraphics() { if (this.texture == null) { this.texture = new VideoTexture(ResourceLocation.fromNamespaceAndPath("mcedia", "player_" + hashCode())); player.bindTexture(this.texture); } }
    public static long parseToMicros(String timeStr) { if (timeStr == null || timeStr.isEmpty()) { throw new IllegalArgumentException("Time string cannot be null or empty"); } String[] parts = timeStr.split(":"); long totalSeconds = 0; try { if (parts.length == 1) totalSeconds = Long.parseLong(parts[0]) * 3600; else if (parts.length == 2) totalSeconds = Long.parseLong(parts[0]) * 3600 + Long.parseLong(parts[1]) * 60; else if (parts.length == 3) totalSeconds = Long.parseLong(parts[0]) * 3600 + Long.parseLong(parts[1]) * 60 + Long.parseLong(parts[2]); else throw new IllegalArgumentException("Invalid time format: " + timeStr); } catch (NumberFormatException e) { throw new IllegalArgumentException("Invalid number in time string: " + timeStr, e); } return totalSeconds * 1_000_000L; }
    public void updateInputUrl(String content) { try { if (Objects.equals(content, this.inputContent)) { return; } this.inputContent = content; if (content == null || content.isBlank()) { open(null); return; } var args = content.split("\n"); var url = args[0].trim(); if (!url.toLowerCase().startsWith("http")) { LOGGER.warn("无效的输入URL: '{}'，将停止播放。", url); open(null); return; } if (Objects.equals(url, playingUrl)) { return; } open(url); } catch (Exception e) { LOGGER.error("Failed to update input URL", e); } }
    ...
    public void updateQuality(String quality) { if (quality == null || quality.isBlank()) { this.desiredQuality = "自动"; } else { this.desiredQuality = quality.trim(); } }
    private long getBaseDuration() { if (inputContent == null) return 0; long duration = 0; try { var args = inputContent.split("\n"); if (args.length < 2) return 0; duration = parseToMicros(args[1]); } catch (Exception e) { LOGGER.debug("获取base duration失败", e); } return duration; }
    public long getServerDuration() { try { String displayName = entity.getMainHandItem().getDisplayName().getString(); var nameParts = displayName.split(":"); if (nameParts.length > 1) { long startTime = Long.parseLong(nameParts[1].substring(0, nameParts[1].length() - 1)); long duration = System.currentTimeMillis() - startTime; return duration < 1000 ? 0 : duration * 1000; } } catch (Exception e) { return 0; } return 0; }
    public long getDuration() { return getBaseDuration() + getServerDuration(); }
    public void update() { try { var mainHandBook = entity.getItemInHand(InteractionHand.MAIN_HAND); if (mainHandBook.getItem() instanceof WritableBookItem) { var components = mainHandBook.get(DataComponents.WRITABLE_BOOK_CONTENT); updateInputUrl(components != null ? components.getPages(Minecraft.getInstance().isTextFilteringEnabled()).findFirst().orElse(null) : null); } else { updateInputUrl(null); } var offHandBook = entity.getItemInHand(InteractionHand.OFF_HAND); if (offHandBook.getItem() instanceof WritableBookItem) { var components = offHandBook.get(DataComponents.WRITABLE_BOOK_CONTENT); if (!Objects.equals(components, preOffHandBookComponent)) { preOffHandBookComponent = components; if (components != null) { var pages = components.getPages(Minecraft.getInstance().isTextFilteringEnabled()).toList(); if (!pages.isEmpty()) updateOffset(pages.get(0)); if (pages.size() > 1) updateAudioOffset(pages.get(1)); if (pages.size() > 2) updateOther(pages.get(2)); if (pages.size() > 3) { updateQuality(pages.get(3)); } else { updateQuality("自动"); } } } } else { if (preOffHandBookComponent != null) { preOffHandBookComponent = null; resetOffset(); resetAudioOffset(); updateQuality("自动"); } } } catch (Exception ignored) {} }
    ...
    public void render(ArmorStandRenderState state, MultiBufferSource bufferSource, PoseStack poseStack, int i) { var size = state.scale * scale; var audioOffsetRotated = new Vector3f(audioOffsetX, audioOffsetY, audioOffsetZ).rotateY(state.yRot); var volumeFactor = 1 - rotationToFactor(state.leftArmPose.x()); var speedFactor = rotationToFactor(state.leftArmPose.y()); speed = speedFactor < 0.1f ? 1f : (speedFactor > 0.5f ? 1f - (1f - speedFactor) * 2f : (speedFactor - 0.1f) / 0.4f * 8f); player.setSpeed(speed); var volume = volumeFactor * audioMaxVolume; synchronized (player) { Media media = player.getMedia(); if (media != null) { media.uploadVideo(); if (media.getHeight() > 0) { halfW = media.getAspectRatio(); } } else { halfW = 1.777f; } } audioSource.setVolume(volume); audioSource.setRange(audioRangeMin, audioRangeMax); audioSource.setPos(((float) state.x + audioOffsetRotated.x), ((float) state.y + audioOffsetRotated.y), ((float) state.z + audioOffsetRotated.z)); poseStack.pushPose(); poseStack.mulPose(new Quaternionf().rotationYXZ((float) Math.toRadians(-state.yRot), 0, 0)); poseStack.mulPose(new Quaternionf().rotationYXZ((float) Math.toRadians(-state.headPose.x()), (float) Math.toRadians(-state.headPose.y()), (float) Math.toRadians(-state.headPose.z()))); poseStack.translate(offsetX, offsetY + 1.02 * state.scale, offsetZ + 0.6 * state.scale); poseStack.scale(size, size, size); renderScreen(poseStack, bufferSource, i); renderProgressBar(poseStack, bufferSource, player.getProgress(), i); poseStack.popPose(); }
    public void tick() { update(); }

    // 使用 handle() 和 exceptionally() 来分离错误处理

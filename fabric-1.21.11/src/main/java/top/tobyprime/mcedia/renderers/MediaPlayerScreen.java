...
public class MediaPlayerScreen implements IMediaPlayerScreenRenderer {
    ...
    private static final Identifier idleScreen = Identifier.fromNamespaceAndPath("mcedia", "textures/gui/idle_screen.png");
    public float Height;
    ...
    private void renderScreen(VideoTexture texture, PoseStack poseStack, MultiBufferSource bufferSource, int i, MediaPlayer player) {
        VertexConsumer consumer = bufferSource.getBuffer(RenderTypes.entityCutoutNoCull((player.getMedia() != null) ? texture.getResourceLocation() : idleScreen));
    ...
    private void renderLoadingBar(PoseStack poseStack, MultiBufferSource bufferSource, int i) {
        ...
        // 画底色（黑色）
        VertexConsumer black = bufferSource.getBuffer(RenderTypes.debugQuads());
        ...
        VertexConsumer white = bufferSource.getBuffer(RenderTypes.debugQuads());
